    ...
    >
      ...
      <div className="absolute top-14 right-6 z-20 flex items-center gap-2">
        ...
        <button
          ...
          className={`relative flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg backdrop-blur-xl border transition-all hover:scale-105 ${needsLayoutRefresh
      ...
      {currentItems.map(item => {
        ...
        return (
          <div
            key={item.id}
            className={`absolute select-none cursor-pointer ${isDragging ? '' : 'transition-transform'} ${isDragging && isSelected ? 'z-50' : 'z-10'
              }`}
            style={{
              left: horizontalPadding + item.position.x + offset.x,
              top: TOP_OFFSET + item.position.y + offset.y,
              width: ICON_SIZE,
              // 🔥 性能优化：CSS containment 减少浏览器布局/绘制范围
          ...
          >
            {/* 图标容器 */}
            <div
              className={`relative aspect-square rounded-xl overflow-hidden ${isDragging ? '' : 'transition-all duration-200'} ${isSelected
