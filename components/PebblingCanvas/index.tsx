...
// === 拆分模块导入 ===
import type { ImageGenOptions, ImageMetadata, PebblingCanvasProps } from './types';
export type { BatchSavedOptions } from './types';
import { uuid, isValidVideo, isValidImage, base64ToFile, extractImageMetadata, resizeImageClient, truncateAgentResultForPrompt, canvasNodeToWorkflowNode } from './canvasUtils';
import { isApiConfigured, generateCreativeImage, editCreativeImage, generateCreativeText, generateAdvancedLLM } from './canvasApi';
...
const PebblingCanvas: React.FC<PebblingCanvasProps> = ({
  ...
  // 🔥 修复：linkingState ref 镜像，避免 React.memo 导致 handleEndConnection 闭包过期
  const linkingStateRef = useRef(linkingState);
  useEffect(() => { linkingStateRef.current = linkingState; }, [linkingState]);
  ...
  const handleEndConnection = async (targetNodeId: string, portKey?: string) => {
    ...
    // 🔥 使用 ref 读取最新 linkingState，避免 React.memo 闭包过期
    const ls = linkingStateRef.current;
    if (ls.active && ls.fromNode && ls.fromNode !== targetNodeId) {
      const sourceNodeId = ls.fromNode;
      ...
      if (!exists) {
        ...
        if (portKey && targetNode?.type === 'rh-config') {
          toPortOffsetY = ls.currPos.y - targetNode.y;
        }
  ...
  // 🔥 性能优化：预计算所有节点的上游图信息，避免在渲染循环内做 O(N²·M) 图遍历
  const nodeUpstreamInfo = useMemo(() => {
    const info = new Map<string, {
      hasUpstreamImage: boolean;
      imageInputCount: number;
      hasDownstream: boolean;
      incomingConnections: { fromNode: string; toPortKey?: string }[];
    }>();
    for (const node of nodes) {
      // imageInputCount
      const fromIds = new Set(connections.filter(c => c.toNode === node.id).map(c => c.fromNode));
      const imageInputCount = Array.from(fromIds).filter(id =>
        nodes.find(n => n.id === id)?.type === 'image'
      ).length;

      // hasUpstreamImageNode (BFS)
      ...
      while (stack.length > 0) {
        ...
        for (const c of connections) {
          ...
          if (!from) continue;
          if (from.type === 'image') { hasUpstream = true; break; }
      ...
      // hasDownstream
      const hasDownstream = connections.some(c => c.fromNode === node.id);

      // incomingConnections
      const incoming = connections
        .filter(c => c.toNode === node.id)
        .map(c => ({ fromNode: c.fromNode, toPortKey: c.toPortKey }));

      info.set(node.id, {
        hasUpstreamImage: hasUpstream,
        imageInputCount,
        hasDownstream,
  ...
  // 🔥 性能优化：缓存 creativeIdeasForImage，避免在渲染循环内每次创建新数组
  const creativeIdeasForImageMemo = useMemo(() => {
    return creativeIdeas?.map((i) => ({ id: i.id, title: i.title, imageUrl: i.imageUrl })) ?? [];
  }, [creativeIdeas]);

  // 🔥 性能优化：视口裁剪 — 只渲染可见区域内的节点
  const visibleNodes = useMemo(() => {
    const container = containerRef.current;
    if (!container) return nodes;
    const rect = container.getBoundingClientRect();
    const PADDING = 300; // 额外边距，避免边缘节点闪烁
    const viewLeft = (-canvasOffset.x - PADDING) / scale;
    const viewTop = (-canvasOffset.y - PADDING) / scale;
    const viewRight = (-canvasOffset.x + rect.width + PADDING) / scale;
    const viewBottom = (-canvasOffset.y + rect.height + PADDING) / scale;
    return nodes.filter(node => {
      const nodeW = node.width || 300;
      const nodeH = node.height || 300;
      return (node.x + nodeW) > viewLeft && node.x < viewRight
        && (node.y + nodeH) > viewTop && node.y < viewBottom;
    });
  }, [nodes, canvasOffset, scale]);

  return (
    <div
      className={`w-full h-full text-white overflow-hidden relative transition-colors duration-300 ${isLightCanvas ? 'bg-[#f5f5f7]' : 'bg-[#0a0a0f]'
        }`}
    ...
    >
      ...
      >
        {/* Background Grid */}
        <div
          className={`absolute inset-0 pointer-events-none transition-opacity duration-300 ${isLightCanvas ? 'opacity-30' : 'opacity-20'
            }`}
          style={{
        ...
        >
          ...
          {/* Nodes - 🔥 视口裁剪：只渲染可见节点 + 使用预计算的 nodeUpstreamInfo */}
          {visibleNodes.map(node => {
            const upstreamInfo = nodeUpstreamInfo.get(node.id);
            return (
              <CanvasNodeItem
                key={node.id}
                node={node}
                ...
                creativeIdeasForImage={creativeIdeasForImageMemo}
                ...
                hasDownstream={upstreamInfo?.hasDownstream ?? false}
                hasImageInput={upstreamInfo?.hasUpstreamImage ?? false}
                imageInputCount={upstreamInfo?.imageInputCount ?? 0}
                incomingConnections={upstreamInfo?.incomingConnections ?? []}
            ...
            <div className={`px-3 py-2 flex gap-1 flex-wrap border-b flex-shrink-0 ${isLightCanvas ? 'border-gray-100' : 'border-white/5'}`}>
              {[
                ...
                <button
                  ...
                  className={`px-2 py-1 text-[10px] rounded-lg transition-all ${libraryFilter === key
              ...
              {filteredIdeas.length === 0 ? (
                ...
                filteredIdeas.map((idea) => (
                  <button
                    ...
                    className={`w-full text-left p-2 rounded-lg border transition-all cursor-pointer ${idea.isWorkflow
            ...
            <div className={`px-3 py-2 flex gap-1 flex-wrap border-b flex-shrink-0 ${isLightCanvas ? 'border-gray-100' : 'border-white/5'}`}>
              {[
                ...
                <button
                  ...
                  className={`px-2 py-1 text-[10px] rounded-lg transition-all ${mediaLibraryFilter === key
                        ...
                        {list.map((item) => {
                          ...
                          return (
                            <div
                              ...
                              className={`rounded-lg border overflow-hidden cursor-grab active:cursor-grabbing transition-all outline-none ${isLightCanvas ? 'bg-gray-50 border-gray-200 hover:bg-gray-100' : 'bg-white/5 border-white/5 hover:bg-white/10'
                                }`}
      ...
      {/* 底部快捷键提示条 */}
      <div
        className={`absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-3 px-4 py-1.5 rounded-full border pointer-events-none select-none z-30 transition-all duration-300 ${isLightCanvas
